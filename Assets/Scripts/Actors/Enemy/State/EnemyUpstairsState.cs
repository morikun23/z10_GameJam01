...
namespace Z10 {
	public class EnemyUpstairsState : IEnemyState {
		...
		public EnemyUpstairsState(Ladder arg_ladder) {
			m_ladder = arg_ladder;
		}
		...
		public void OnUpdate(ActorEnemy arg_enemy) {
			...
			RaycastHit2D hitInfo = Physics2D.BoxCast(arg_enemy.transform.position ,
				...
				0 , Vector2.up , 0.5f , 1 << LayerMask.NameToLayer("Player"));

			if (hitInfo) {
				ActorBuddy player = hitInfo.collider.gameObject.GetComponent<ActorBuddy>();
				if (player) {
					if (player.m_currentState.GetType() == typeof(BuddyDownAttackState)) { }
					else {
						player.OnDamaged();
					}
				}
